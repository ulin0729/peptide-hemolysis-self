...
class HemolysisDataset(Dataset):
    ...
    def __getitem__(self, index):
        return self.features[index], self.conc[index], self.label[index]
...
# print(f'train #: {len(train_subset)}, validate #: {len(validate_subset)}')
# valid_n = len(validate_subset)
...
# ValidateLoader = DataLoader(dataset=validate_subset, batch_size=10000000, shuffle=False, num_workers=0)
...
for k in range(k_fold):
    ...
    trrr = total_size
    # msg
    print("train indices: [%d,%d),[%d,%d), valid indices: [%d,%d)"%(trll,trlr,trrl,trrr,vall,valr))
    ...
    validate_subset = Subset(dataset,val_indices)
    valid_n = len(validate_subset)
    TrainLoader = DataLoader(dataset=train_subset, batch_size=batch_size, shuffle=True, num_workers=0, generator=torch.Generator().manual_seed(seed))
    ValidateLoader = DataLoader(dataset=validate_subset, batch_size=10000000, shuffle=False, num_workers=0)
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, 10, 5, 0, -1, False)
    ...
    for epoch in range(num_epochs):
        ...
        if acc > best_acc:
            best_acc = acc
            torch.save(model.state_dict(), model_path+f'{k}') # Save your best model
            print('Saving model with accuracy {:.4f}...'.format(best_acc))
            early_stop_count = 0
        else: 
            early_stop_count += 1
        ...
        print(f'Epoch [{epoch+1}/{num_epochs}], Average Training Loss: {total_loss/total_steps:.4f}, Average Validation Loss: {total_val_loss/total_val_steps:.4f}, lr: {scheduler.get_last_lr()[0]:.4f}, acc: {acc:.6f}, {correct}/{valid_n}')
        ...
        if (epoch+1) % 500 == 0:
            ...
            plt.xlabel('n epoch')
            plt.savefig(f'{k}_epoch{epoch+1}.png')
    ...
    TrainLoader = DataLoader(dataset=train_subset, batch_size=10000000, shuffle=False, num_workers=0)
